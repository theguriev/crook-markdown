...
///
/// **3** is the version a plugin can do something *about one command* in.
/// Everything before it was about the window: a chip in the header, a panel
/// under it, a request for a file or a host. This one adds the block —
/// [`Capability::ReadBlock`], [`Request::Block`] and the [`Answer::Block`] it
/// comes back as — and [`Request::Copy`], which is the shortest way for a
/// plugin to hand a person something it worked out. A plugin that contributes
/// to `block.menu` and asks for neither is still a plugin: an entry that runs
/// an action it already knows how to do needs nothing here.
///
/// **2** was the version a plugin could *do* something in. One added
...
/// The oldest version this vocabulary can still read.
///
/// A host refuses what it cannot read, and what it *can* read is decided by
/// one property of every version since this one: **everything added is added
/// at the end.** postcard writes a variant as its index, so an enum whose new
/// variants are appended decodes an older plugin's messages to exactly what
/// that plugin meant — every index it can send is a variant that has not
/// moved. The danger runs the other way, and it is the one the version number
/// exists for: an older *host* reading a newer plugin would read an index it
/// has no variant for, or the wrong one.
///
/// So a plugin is refused for being newer, not for being older. A plugin
/// written against 2 goes on working in a host that speaks 3 and simply never
/// asks for the things 3 added — which is the difference between an ABI that
/// can grow and one where every addition costs everybody a release.
///
/// Two rather than one, because version 1's [`Node`] was not a prefix of
/// version 2's: the variants a panel needed were put in among the ones that
/// were already there rather than after them, so a 1 module and a 2 host
/// disagree about what a `Row` is. The rule above starts here.
pub const OLDEST_ABI: u32 = 2;
...
pub enum Capability {
    ...
    /// Read the command a block ran, what it printed, and where.
    ///
    /// Only the block a menu of this plugin's is open on, and only while that
    /// entry is running: this is not a way to read a session. A plugin with
    /// this capability sees one command at a time, and only ever one a person
    /// pointed at it.
    ///
    /// Last in the list, and that is a rule rather than a coincidence: see
    /// [`OLDEST_ABI`]. A variant added anywhere else would renumber the ones
    /// after it, and a plugin built against an older version of this file
    /// would then be read as asking for something it never asked for.
    ReadBlock,
}
...
pub enum Request {
    ...
    /// What the block this plugin was just run on ran, and what it printed.
    /// Needs [`Capability::ReadBlock`].
    ///
    /// There is no way to name a block and no way to ask for another one. The
    /// answer is about the block whose menu the person used to run this
    /// plugin's entry, and asking outside that — from a tick, from a chip in
    /// the header — comes back [`Answer::Failed`] rather than with whatever
    /// block happens to be under the pointer.
    Block,
    /// Put text on the system clipboard. Needs [`Capability::Clipboard`].
    ///
    /// The shortest way for a plugin to hand a person something it worked out,
    /// and the reason it is a request rather than a [`Node`]: what goes on the
    /// clipboard is a thing that *happens*, at a moment, because somebody
    /// asked — not a thing that is drawn.
    Copy {
        /// What to put there.
        text: String,
    },
...
pub enum Answer {
    ...
    /// The block the plugin was run on.
    ///
    /// Every field is an [`Option`] because every one of them is something a
    /// shell may not have said: a command line needs the `B` mark or a submit,
    /// the output needs `C`, the status needs `D`, and the directory and the
    /// branch need a shell that reports where it is. A plugin that treats a
    /// missing field as an error will be wrong on somebody's `ssh`.
    Block {
        /// The command line, as it was submitted or echoed.
        command: Option<String>,
        /// What the command printed, without the prompt or the line it was
        /// typed on.
